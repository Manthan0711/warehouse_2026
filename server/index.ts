...
const applyBlockStateTransition = async (
  ...
  const updatedBlocks = warehouseData.blocks.map((block: any) => {
    ...
    return {
      ...
      booked_by:
        bookingMeta?.customer_details?.email ||
        bookingMeta?.customer_details?.name ||
        block.booked_by,
      booking_dates: {
      ...
      reserved_at:
        nextBlockState === "reserved"
          ? new Date().toISOString()
          : block.reserved_at || new Date().toISOString(),
      occupied_at:
        nextBlockState === "occupied" ? new Date().toISOString() : null,
    };
  });
  ...
  try {
    ...
    if (error) {
      ...
      return res.json({
        ...
        bookings: [],
        total: 0,
        message: "No bookings found or table does not exist",
      });
    }
...
const updateBookingStatus: RequestHandler = async (req, res) => {
  try {
    ...
    if (!bookingId || !status) {
      return res
        .status(400)
        .json({ success: false, error: "Missing bookingId or status" });
    }
    ...
    if (existing.metadata?.warehouse_id) {
      ...
      const blockIds = normalizeBookingBlockIds(
        existing.metadata.blocks_booked || [],
      );
      if (blockIds.length > 0) {
        await applyBlockStateTransition(
          warehouseId,
          blockIds,
          status,
          existing.metadata,
    ...
    if (
      ...
      existing.metadata?.warehouse_id
    ) {
      const warehouseId = existing.metadata.warehouse_id;
      ...
      if (ownerId) {
        ...
        console.log(
          `   Booking metadata warehouse_owner_id: ${existing.metadata?.warehouse_owner_id}`,
        );
      }
    }
...
export function createServer() {
  ...
  app.get("/api/admin/warehouse-submissions", async (req, res) => {
    try {
      const { getWarehouseSubmissions } =
        await import("./routes/admin-warehouse-submissions");
  ...
  app.post("/api/admin/warehouse-submissions/review", async (req, res) => {
    try {
      const { reviewWarehouseSubmission } =
        await import("./routes/admin-warehouse-submissions");
  ...
  app.get("/api/admin/user-activity", async (req, res) => {
    try {
      const { getAdminUserActivity } =
        await import("./routes/admin-user-activity");
  ...
  app.get("/api/saved/:seekerId", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, warehouses: [], error: "Route loading error" });
    }
  });
  ...
  app.get("/api/saved/:seekerId/status/:warehouseId", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, saved: false, error: "Route loading error" });
    }
  });
  ...
  app.get("/api/activity/:seekerId", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, activities: [], error: "Route loading error" });
    }
  });

...
const updateBlocksForOwnerAction = async (
  ...
  if (byWhId) {
    ...
    const { data: sub } = await supabase
      ...
      .eq('status', 'approved')
      .maybeSingle();
    if (sub) { warehouseData = sub; table = 'warehouse_submissions'; col = 'id'; }
  }

  if (!warehouseData || !Array.isArray(warehouseData.blocks) || !warehouseData.blocks.length) return;
  ...
  const updatedBlocks = warehouseData.blocks.map((block: any) => {
    ...
    if (nextState === 'available') {
      return { ...block, status: 'available', booking_id: null, booked_by: null, booking_dates: null, reserved_at: null, occupied_at: null };
    }
    return {
      ...block,
      status: nextState,
      ...
      booking_dates: { start: bookingMeta?.start_date, end: bookingMeta?.end_date },
  ...
  const unavailable = updatedBlocks.filter((b: any) => ['reserved', 'occupied', 'booked'].includes(String(b.status))).length;
...
export const getOwnerBookings: RequestHandler = async (req, res) => {
  ...
  if (!owner_id) return res.status(400).json({ success: false, error: 'Missing owner_id' });
  ...
  try {
    ...
    const [{ data: warehouses }, { data: submissions }] = await Promise.all([
      supabase.from('warehouses').select('id, wh_id, name').eq('owner_id', String(owner_id)),
      supabase.from('warehouse_submissions').select('id, name').eq('owner_id', String(owner_id)).eq('status', 'approved'),
    ]);
    ...
    (submissions || []).forEach(s => { if (s.id) warehouseIdSet.add(String(s.id)); });
...
export const respondToBooking: RequestHandler = async (req, res) => {
  ...
  if (!booking_id || !owner_id || !['approve', 'reject'].includes(action)) {
    return res.status(400).json({ success: false, error: 'Missing booking_id, owner_id, or invalid action' });
  }
  ...
  try {
    ...
    if (currentStatus !== 'pending') {
      return res.status(409).json({ success: false, error: `Booking is already ${currentStatus}. Only pending bookings can be responded to.` });
    }

    const warehouseId = booking.metadata?.warehouse_id;
    if (!warehouseId) {
      return res.status(400).json({ success: false, error: 'Booking missing warehouse_id' });
    }

    // Verify that this owner owns the warehouse
    const [{ data: mainWh }, { data: subWh }] = await Promise.all([
      supabase.from('warehouses').select('id').or(`wh_id.eq.${warehouseId},id.eq.${warehouseId}`).eq('owner_id', owner_id).maybeSingle(),
      supabase.from('warehouse_submissions').select('id').eq('id', warehouseId).eq('owner_id', owner_id).maybeSingle(),
    ]);

    if (!mainWh && !subWh) {
      return res.status(403).json({ success: false, error: 'Not authorized to manage this warehouse' });
    }
    ...
    if (blockIds.length > 0) {
      await updateBlocksForOwnerAction(warehouseId, blockIds, newStatus, booking.metadata, booking_id);
    }
    ...
    return res.json({ success: true, message: `Booking ${newStatus} by owner`, status: newStatus });
  } catch (err) {

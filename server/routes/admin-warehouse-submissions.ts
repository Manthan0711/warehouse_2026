...
const toTextArray = (val: any): string[] => {
  ...
  if (typeof val === "object")
    return Object.values(val).filter(Boolean).map(String);
  return [];
};
...
export const getWarehouseSubmissions: RequestHandler = async (_req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to load submissions" });
  }
};

export const reviewWarehouseSubmission: RequestHandler = async (req, res) => {
  try {
    const { submissionId, decision, adminNotes, rejectionReason, reviewedBy } =
      req.body || {};
    ...
    if (decision === "approved") {
      ...
      if (reinsertError) {
        ...
        return res
          .status(500)
          .json({ success: false, error: reinsertError.message });
      }

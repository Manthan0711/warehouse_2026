...
const statusUsesCapacity = (status?: string) =>
  ["pending", "approved", "confirmed", "active"].includes(status || "");

const normalizeBlocks = (
  rawBlocks: any[],
): Array<{ id: string; block_number: number; area: number }> => {
...
const getBlockIds = (blocks: any[]): string[] =>
  normalizeBlocks(blocks).map((b) => b.id);
...
const getUnavailableBlockIds = (
  ...
  for (const row of bookingLogs || []) {
    ...
    if (!isDateOverlap(startDate, endDate, md.start_date, md.end_date))
      continue;
    ...
    return {
      ...
      reserved_at:
        nextState === "reserved"
          ...
          : block.reserved_at || new Date().toISOString(),
      occupied_at:
        nextState === "occupied"
          ? new Date().toISOString()
          : block.occupied_at || null,
    };
  });
...
export const bookWarehouseBlocks: RequestHandler = async (req, res) => {
  try {
    ...
    if (!normalizedBlocks.length) {
      return res
        .status(400)
        .json({ success: false, error: "At least one block must be selected" });
    }

    const totalArea = normalizedBlocks.reduce(
      (sum: number, block: any) => sum + (block.area || 100),
      0,
    );
    ...
    if (mainWarehouse) {
      ...
      console.log(
        `✅ Found warehouse in main table, owner_id: ${warehouseOwnerId}`,
      );
    } else {
      ...
      if (submissionWarehouse) {
        ...
        console.log(
          `✅ Found warehouse in submissions, owner_id: ${warehouseOwnerId}`,
        );
      }
    }
    ...
    if (!warehouse) {
      console.log(
        `⚠️ Warehouse ${warehouse_id} not found in database, using provided details`,
      );
    }
    ...
    if (existingError) {
      return res
        ...
        .json({
          success: false,
          error: "Failed to validate block availability",
          details: existingError.message,
        });
    }

    const unavailableIds = getUnavailableBlockIds(
      existingBookings || [],
      start_date,
      end_date,
    );
    ...
    if (conflicting.length > 0) {
      return res.status(409).json({
        success: false,
        error:
          "Some selected blocks were just reserved by another booking. Please refresh availability.",
    ...
    await updateWarehouseBlockState(warehouse_id, requestedIds, "reserved", {
  ...
  try {
    ...
    const filteredBookings = (bookings || []).filter((row: any) => {
      ...
      return isDateOverlap(
        String(start_date),
        String(end_date),
        row.metadata?.start_date,
        row.metadata?.end_date,
      );
    });

    filteredBookings.forEach((booking: any) => {
      getBlockIds(booking.metadata?.blocks_booked || []).forEach((id) =>
        bookedBlocks.add(id),
      );
    });
    ...
    console.log(
      `✅ Found ${blocks.filter((b) => b.available).length} available blocks`,
    );

...
async function createMockDataFile() {
  ...
  for await (const row of parser) {
    ...
    const warehouse = {
      ...
      description:
        row.Description || "General warehouse for storage and distribution",
      ...
      total_area:
        parseFloat(row.Total_Area_sqft) ||
        10000 + Math.floor(Math.random() * 90000),
      price_per_sqft:
        parseFloat(row.Price_Per_Sqft) || 15 + Math.floor(Math.random() * 35),
      images: [
  ...
  const mockDataPath = join(
    ...
    "client",
    "data",
    "mock-warehouses.json",
  );
  fs.writeFileSync(mockDataPath, JSON.stringify(warehouses, null, 2));

  console.log(
    `Created mock data with ${warehouses.length} warehouses at ${mockDataPath}`,
  );
  return warehouses;
}

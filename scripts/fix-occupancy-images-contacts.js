...
function generateEmail(name) {
  ...
  const cleanName = (name || "warehouse")
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "")
    .substring(0, 15);
  ...
  try {
    ...
    while (processed < totalCount) {
      ...
      const { data: warehouses, error: fetchError } = await supabase
        .from("warehouses")
        .select(
          "id, warehouse_type, total_area, occupancy, owner_name, owner_email, owner_phone, images",
        )
      ...
      for (const wh of warehouses) {
        ...
        const hasGeneric = currentImages.some(
          (img) =>
        ...
        const needsImageUpdate =
          ...
          (warehouseType !== "Automobile Spare Storage" &&
            currentImages.some(
              (img) => img.includes("car") || img.includes("552519507"),
            ));
        ...
        if (
          !wh.owner_name ||
          wh.owner_name.startsWith("Owner ") ||
          wh.owner_name === "null"
        ) {
        ...
        if (
          !wh.owner_email ||
          wh.owner_email.includes("@example.com") ||
          wh.owner_email === "null"
        ) {
          updates.owner_email = generateEmail(
            updates.owner_name || wh.owner_name || "Warehouse",
          );
        }
    ...
    const { data: sample } = await supabase
      .from("warehouses")
      .select(
        "id, name, warehouse_type, occupancy, available_area, owner_name, owner_email, owner_phone, images",
      )
      .limit(5);
    ...
    if (occupancyStats) {
      const occupancies = occupancyStats.map(
        (w) => parseFloat(w.occupancy) || 0,
      );

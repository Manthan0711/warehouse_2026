...
async function importWarehouses() {
  ...
  console.log(
    `📦 Total warehouses to import: ${allMaharashtraWarehouses.length}`,
  );

  // Convert all warehouses
  const warehousesToImport = allMaharashtraWarehouses.map(
  ...
  for (let i = 0; i < warehousesToImport.length; i += batchSize) {
    ...
    try {
      const { data, error } = await supabase.from("warehouses").upsert(batch, {
        onConflict: "wh_id",
    ...
    // Progress update
    const progress = (
      ((i + batch.length) / warehousesToImport.length) *
      100
    ).toFixed(1);
    console.log(

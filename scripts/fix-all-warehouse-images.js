...
function getRandomImages(warehouseType, warehouseId) {
  ...
  const seed =
    warehouseId.charCodeAt(0) + warehouseId.charCodeAt(warehouseId.length - 1);
  ...
  while (images.length < 2) {
    ...
    if (!images.includes(randomImg)) {
      ...
      images.push(
        DEFAULT_WAREHOUSE_IMAGES[
          images.length % DEFAULT_WAREHOUSE_IMAGES.length
        ],
      );
    }
  }
...
async function fixAllImages() {
  ...
  for (let offset = 0; offset < count; offset += batchSize) {
    ...
    process.stdout.write(
      `\r📦 Progress: ${processed}/${count} (${progress}%) | Updated: ${updated} | Errors: ${errors}`,
    );
  }

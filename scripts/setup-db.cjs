...
async function createWarehousesTable() {
  ...
  try {
    ...
    if (
      error &&
      error.message.includes('relation "public.warehouses" does not exist')
    ) {
      console.log(
        "❌ Warehouses table does not exist. It needs to be created manually in Supabase.",
      );
      console.log(
        "📋 Please go to your Supabase dashboard and create the table with the following structure:",
      );
      console.log(`
...
async function testInsert() {
  ...
  try {
    ...
    if (error) {
      ...
      await supabase.from("warehouses").delete().eq("id", data[0].id);

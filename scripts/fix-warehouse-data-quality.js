...
function getRandomCoordinates(city) {
  const location =
    MAHARASHTRA_LOCATIONS[city] || MAHARASHTRA_LOCATIONS["Mumbai"];
  ...
  try {
    ...
    while (processed < totalCount) {
      ...
      const { data: warehouses, error: fetchError } = await supabase
        .from("warehouses")
        .select(
          "id, city, district, latitude, longitude, pincode, owner_name, owner_email, owner_phone, total_area, price_per_sqft",
        )
      ...
      for (const wh of warehouses) {
        ...
        if (needsUpdate) {
          ...
          if (updateError) {
            console.error(
              `Error updating warehouse ${wh.id}:`,
              updateError.message,
            );
            errors++;
          } else {
...
async function main() {
  ...
  if (args.includes("--verify")) {
    ...
    console.log("\nWould fix:");
    console.log(
      "- Missing latitude/longitude with realistic Maharashtra coordinates",
    );

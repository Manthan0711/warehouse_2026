...
async function updateContacts() {
  ...
  const { data: warehouses, error } = await supabase
    .from("warehouses")
    .select(
      "id, city, warehouse_type, contact_phone, contact_email, contact_person",
    )
  ...
  console.log(
    `📊 Found ${warehouses.length} warehouses with missing contact info\n`,
  );
  ...
  for (let i = 0; i < warehouses.length; i += batchSize) {
    ...
    process.stdout.write(
      `\r📦 Progress: ${progress}/${warehouses.length} (${((progress / warehouses.length) * 100).toFixed(1)}%) | Updated: ${updated} | Errors: ${errors}`,
    );
  }

...
async function main() {
  ...
  if (!filePathArg || !submissionId) {
    console.error(
      "Usage: node example-client.js <path-to-file> <submission-id>",
    );
    process.exit(1);
  }
  ...
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.log(
      "SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY not set; skipping DB update. Use the returned publicUrl to update a submission record manually.",
    );
    process.exit(0);
  }

  const updateResp = await fetch(
    `${SUPABASE_URL}/rest/v1/warehouse_submissions?id=eq.${submissionId}`,
    {
      method: "PATCH",
      headers: {
        apikey: SUPABASE_KEY,

 ...
 */
function filterByLicenseValidity(
  ...
  return warehouses.filter((warehouse) => {
    ...
    if (licenseStatus.shouldExclude) {
      console.log(
        `⚠️ Excluding warehouse ${warehouse.name} - license expired or missing`,
      );
      return false;
    }
...
export function knnRecommend(
  ...
  const warehousesWithScores = warehouses.map((warehouse) => {
    ...
    const warehouseFeatures = {
      ...
      priceScore:
        targetPrice && targetPrice > 0
          ? warehouse.price_per_sqft <= targetPrice
            ? Math.max(
                0.5,
                1 - (targetPrice - warehouse.price_per_sqft) / targetPrice,
              ) // Lower price is good but not too low
            : Math.max(
                0,
                1 - (warehouse.price_per_sqft - targetPrice) / targetPrice,
              )
          : 0.5, // Higher price is worse

      // Area feature
      areaScore:
        minAreaSqft && minAreaSqft > 0
          ? warehouse.total_area >= minAreaSqft
            ? Math.min(
                1.0,
                1 -
                  Math.abs(warehouse.total_area - minAreaSqft * 1.2) /
                    (minAreaSqft * 2),
              )
            : 0.2
          : 0.5,
      ...
      typeScore:
        preferredType && preferredType !== "any"
          ? warehouse.features?.some((f) =>
              f.toLowerCase().includes(preferredType.toLowerCase()),
            ) ||
            warehouse.amenities?.some((a) =>
              a.toLowerCase().includes(preferredType.toLowerCase()),
            )
            ? 1.0
            : 0.1
          : 0.5,
      ...
      verificationScore: warehouse.amenities?.some((a) =>
      ...
      ratingScore: warehouse.rating
        ? Math.min(1.0, warehouse.rating / 5.0)
        : 0.5,

      // Amenities score - more amenities is better
      amenitiesScore: warehouse.amenities
        ? Math.min(1.0, warehouse.amenities.length / 10.0)
        : 0.3,
    };
    ...
    for (const [feature, value] of Object.entries(warehouseFeatures) as [
      keyof typeof warehouseFeatures,
      number,
    ][]) {
    ...
    const normalizedDistance =
      Math.sqrt(squaredDistanceSum) / Math.sqrt(weightSum);
    ...
    if (targetPrice && warehouse.price_per_sqft <= targetPrice) {
      const pctBelow = Math.round(
        (1 - warehouse.price_per_sqft / targetPrice) * 100,
      );
      if (pctBelow >= 5) {
    ...
    if (minAreaSqft && warehouse.total_area >= minAreaSqft) {
      const pctAbove = Math.round(
        (warehouse.total_area / minAreaSqft - 1) * 100,
      );
      if (pctAbove > 10 && pctAbove < 50) {
        reasons.push(
          `${pctAbove}% more space than required (${warehouse.total_area.toLocaleString()} sqft)`,
        );
      } else {
        reasons.push(
          `Optimal size for your needs (${warehouse.total_area.toLocaleString()} sqft)`,
        );
      }
    }
    ...
    if (warehouseFeatures.availabilityScore >= 0.7 && preferAvailability) {
      reasons.push(
        `High availability (${Math.round(warehouseFeatures.availabilityScore * 100)}% free space)`,
      );
    }
 ...
 */
export function randomForestRecommend(
  warehouses: SupabaseWarehouse[],
  preferences: RecommendationPreferences,
  numTrees: number = 50,
  samplesPerTree: number = 0.8,
): SupabaseWarehouse[] {
  if (!warehouses || warehouses.length === 0) return [];
  ...
  const allFeatures: Feature[] = [
      ...
      extractor: (warehouse) => {
        ...
        const exactMatch =
          warehouse.city?.toLowerCase() ===
            preferences.district.toLowerCase() ||
          warehouse.district?.toLowerCase() ===
            preferences.district.toLowerCase();

        return exactMatch ? 1.0 : 0.0;
      },
      importance:
        preferences.district && preferences.district !== "any" ? 0.5 : 0.05,
    },
    {
      name: "priceMatch",
      extractor: (warehouse) => {
        if (!preferences.targetPrice || preferences.targetPrice <= 0)
          return 0.5;
        ...
        if (priceDiff <= -0.2)
          return 0.9; // 20%+ below budget
        else if (priceDiff <= -0.1)
          return 0.85; // 10-20% below budget
        else if (priceDiff <= 0)
          return 0.8; // 0-10% below budget
        else if (priceDiff <= 0.1)
          return 0.6; // 0-10% above budget
        else if (priceDiff <= 0.2)
          return 0.4; // 10-20% above budget
        else if (priceDiff <= 0.3)
          return 0.2; // 20-30% above budget
        else return 0.1; // 30%+ above budget
      },
      importance:
        preferences.targetPrice && preferences.targetPrice > 0 ? 0.25 : 0.05,
    },
    {
      name: "areaMatch",
      extractor: (warehouse) => {
        ...
        if (areaRatio < 0.8)
          ...
          return 0.4; // Slightly too small
        else if (areaRatio <= 1.2)
          return 0.95; // Perfect size
        else if (areaRatio <= 1.5)
          return 0.8; // Slightly larger
        else if (areaRatio <= 2.0)
          return 0.6; // Moderately larger
        else if (areaRatio <= 3.0)
          return 0.4; // Much larger
        else return 0.2; // Excessively large
      },
      importance:
        preferences.minAreaSqft && preferences.minAreaSqft > 0 ? 0.2 : 0.05,
    },
    {
      name: "typeMatch",
      extractor: (warehouse) => {
    ...
    },
    {
      ...
      extractor: (warehouse) => {
        ...
        if (availability >= 0.8)
          return 0.95; // 80%+ available
        else if (availability >= 0.6)
          return 0.8; // 60-80% available
        else if (availability >= 0.4)
          return 0.6; // 40-60% available
        else if (availability >= 0.2)
          return 0.4; // 20-40% available
    ...
    },
    {
      ...
      extractor: (warehouse) => {
        ...
        const amenityScore = Math.min(
          1.0,
          warehouse.amenities.length * 0.07 + premiumCount * 0.12,
        );
        return amenityScore;
      },
      importance: 0.08,
    },
    {
      ...
      extractor: (warehouse) => {
        if (!warehouse.reviews_count || warehouse.reviews_count === 0)
          return 0.3;
  ...
  for (let treeIndex = 0; treeIndex < numTrees; treeIndex++) {
    ...
    const numFeaturesToSelect = Math.max(
      1,
      Math.floor(Math.sqrt(allFeatures.length)),
    );
    ...
    const scored = sample.map((warehouse) => {
      ...
      const normalizedScore =
        totalImportance > 0 ? score / totalImportance : 0.5;
    ...
    scored.slice(0, topK).forEach((item, index) => {
      ...
      if (
        ...
        warehouseInsights.add(
          `Perfect location match in ${item.warehouse.city}`,
        );
      } else if (
        preferences.district &&
      ...
      if (
        preferences.targetPrice &&
        item.warehouse.price_per_sqft <= preferences.targetPrice
      ) {
        const pctBelow = Math.round(
          (1 - item.warehouse.price_per_sqft / preferences.targetPrice) * 100,
        );
        if (pctBelow >= 5) {
          warehouseInsights.add(`${pctBelow}% below your target budget`);
        } else {
          warehouseInsights.add(
            `Within your budget at ₹${item.warehouse.price_per_sqft}/sqft`,
          );
        }
      }
      ...
      if (
        ...
        item.warehouse.total_area >= preferences.minAreaSqft
      ) {
      ...
      if (
        ...
        (item.warehouse.features?.some((f) =>
          f.toLowerCase().includes(preferences.preferredType!.toLowerCase()),
        ) ||
          item.warehouse.amenities?.some((a) =>
            a.toLowerCase().includes(preferences.preferredType!.toLowerCase()),
          ))
      ) {
        warehouseInsights.add(
          `Specialized for ${preferences.preferredType} storage`,
        );
      }

      if (item.warehouse.rating && item.warehouse.rating >= 4.5) {
        warehouseInsights.add(
          `Exceptional ratings (${item.warehouse.rating}⭐)`,
        );
      }

      if (
        preferences.preferVerified &&
        item.warehouse.amenities?.some((a) =>
          a.toLowerCase().includes("verified"),
        )
      ) {
      ...
      if (
        preferences.preferAvailability &&
        item.warehouse.occupancy !== undefined
      ) {
      ...
      if (item.warehouse.amenities && item.warehouse.amenities.length >= 5) {
        warehouseInsights.add(
          `Well-equipped with ${item.warehouse.amenities.length} amenities`,
        );
      }
    });
  }
  ...
  const results = Array.from(votes.entries()).map(
    ([warehouseId, voteCount]) => {
      ...
      // Get insights
      const warehouseInsights = Array.from(
        insights.get(warehouseId) || new Set<string>(),
      );

      // Make sure we have at least some insights
      if (warehouseInsights.length === 0) {
        warehouseInsights.push(
          `Located in ${warehouse.city}, ${warehouse.state}`,
        );
      }
...
export function hybridRecommend(
  ...
  console.log(
    `🔒 License filtering: ${warehouses.length} → ${validLicenseWarehouses.length} warehouses (removed ${warehouses.length - validLicenseWarehouses.length} with expired/missing licenses)`,
  );
  ...
  if (preferences.district && preferences.district !== "any") {
    ...
    if (primaryWarehouses.length < 5 && secondaryWarehouses.length > 0) {
      ...
      primaryWarehouses = [
        ...primaryWarehouses,
        ...secondaryWarehouses.slice(0, 50),
      ];
    }
  }
  ...
  function computeMetaFeatures(warehouse: SupabaseWarehouse): MetaFeatures {
    ...
    return {
      ...
      rankDifference: Math.abs(
        (knnRank >= 0 ? knnRank : knnResults.length) -
          (rfRank >= 0 ? rfRank : rfResults.length),
      ),
      normalizedRating: warehouse.rating
        ? Math.min(1, warehouse.rating / 5)
        : 0.5,
      ...
      priceMatchScore: (() => {
        ...
        const priceDiff =
          ...
          preferences.targetPrice;
        if (priceDiff <= -0.15)
          return 0.9; // 15%+ below target (great value)
        else if (priceDiff <= -0.05)
          return 0.8; // 5-15% below target (good value)
        else if (priceDiff <= 0.05)
          return 0.7; // Within 5% of target (close match)
        else if (priceDiff <= 0.15)
          return 0.5; // 5-15% above target (acceptable)
        else if (priceDiff <= 0.3)
          return 0.3; // 15-30% above target (expensive)
      ...
      areaMatchScore: (() => {
        ...
        if (areaRatio < 0.5)
          return 0.0; // Way too small - exclude
        else if (areaRatio < 0.8)
          return 0.05; // Too small - heavy penalty
        else if (areaRatio < 0.95)
          return 0.2; // Slightly too small
        else if (areaRatio <= 1.2)
          return 1.0; // Perfect size - meets requirement
        else if (areaRatio <= 1.5)
          return 0.9; // Slightly larger - still great
        else if (areaRatio <= 2.0)
          return 0.75; // Moderately larger
        else if (areaRatio <= 3.0)
          return 0.6; // Much larger
      ...
      // Amenity richness
      amenityRichness: warehouse.amenities
        ? Math.min(1.0, warehouse.amenities.length / 10)
        : 0.2,
  ...
  const results = Array.from(allIds).map((id) => {
    ...
    const weights = {
      ...
      ratingWeight: 0.8,
      locationWeight:
        preferences.district && preferences.district !== "any" ? 5.0 : 0.5,
      priceWeight:
        preferences.targetPrice && preferences.targetPrice > 0 ? 2.0 : 0.5,
      areaWeight:
        preferences.minAreaSqft && preferences.minAreaSqft > 0 ? 1.5 : 0.5,
      typeWeight:
        preferences.preferredType && preferences.preferredType !== "any"
          ? 1.5
          : 0.3,
    ...
    if (features.inBothAlgorithms) {
      ...
      algorithmAgreementScore = features.inBothAlgorithms
        ? 0.5 + normalizedRankDiff * 0.5
        : 0;
    ...
    if (
      preferences.district &&
      preferences.district !== "any" &&
      features.locationMatchScore < 0.5
    ) {
    ...
    const calibratedScore = Math.min(
      ...
      Math.max(
        0.45,
        penalizedScore < 0.5
          ? penalizedScore // Keep poor matches low
          : 0.5 + (penalizedScore - 0.5) * 0.96, // Scale 0.5-1.0 to 0.50-0.98
      ),
    );
    ...
    const featureStrengths = [
      {
        name: "location",
        score: features.locationMatchScore * weights.locationWeight,
        reason: `Perfect location match in ${warehouse.city}`,
      },
      {
        name: "price",
        score: features.priceMatchScore * weights.priceWeight,
        reason:
          preferences.targetPrice &&
          warehouse.price_per_sqft <= preferences.targetPrice
      ...
      {
        name: "area",
        score: features.areaMatchScore * weights.areaWeight,
        reason:
          preferences.minAreaSqft &&
          warehouse.total_area >= preferences.minAreaSqft
      ...
      {
        name: "type",
        score: features.typeMatchScore * weights.typeWeight,
        reason: preferences.preferredType
          ? `Specialized for ${preferences.preferredType} storage`
          : warehouse.features && warehouse.features.length > 0
            ? `Features: ${warehouse.features[0]}`
            : "",
      },
      {
        name: "rating",
        score: features.normalizedRating * weights.ratingWeight,
        reason:
          warehouse.rating >= 4.7
            ? `Exceptional ratings (${warehouse.rating}⭐)`
            : warehouse.rating >= 4.3
              ? `Highly rated (${warehouse.rating}⭐)`
              : "",
      },
      {
        name: "verification",
        score: features.verificationScore * weights.verificationWeight,
        reason:
          features.verificationScore > 0.7
            ? "Verified facility with proper documentation"
            : "",
      },
      {
        name: "amenities",
        score: features.amenityRichness * weights.amenityWeight,
        reason:
          warehouse.amenities && warehouse.amenities.length >= 5
            ? `Well-equipped with ${warehouse.amenities.length} amenities`
            : "",
      },
      {
        name: "availability",
        score: features.availabilityScore * weights.availabilityWeight,
        reason:
          features.availabilityScore >= 0.7
    ...
    let recommendationType: "ml" | "knn" | "random-forest" | "hybrid" =
      "hybrid";
  ...
  for (let iter = 0; iter < numIterations; iter++) {
    ...
    const featureScores = warehouses.map((warehouse, i) => {
      const features = {
        ...
        area:
          minAreaSqft && minAreaSqft > 0
            ? (warehouse.total_area || 0) >= minAreaSqft
              ? 1
              : 0
            : 0.5,
        price:
          targetPrice && targetPrice > 0
            ? Math.max(
                0,
                1 -
                  Math.abs(warehouse.price_per_sqft - targetPrice) /
                    targetPrice,
              )
            : 0.5,
      ...
      return (
        features.location * 0.4 +
        features.area * 0.25 +
        features.price * 0.2 +
        features.rating * 0.1 +
        features.availability * 0.05
      );
    });
    ...
    predictions = predictions.map((pred, i) =>
      Math.min(
        ...
        Math.max(
          0,
          pred +
            learningRate * featureScores[i] * (residuals[i] > 0 ? 1 : -0.5),
        ),
      ),
    );
  }
...
export function neuralNetworkScoring(
  ...
  const results = warehouses.map((warehouse) => {
    ...
    const inputs = {
      locationMatch:
        district && district !== "any"
          ? warehouse.city?.toLowerCase() === district.toLowerCase() ||
            warehouse.district?.toLowerCase() === district.toLowerCase()
            ? 1
            : warehouse.district?.toLowerCase().includes(district.toLowerCase())
              ? 0.5
              : 0
          : 0.5,
      ...
      priceNormalized:
        targetPrice && targetPrice > 0
          ? sigmoid((targetPrice - warehouse.price_per_sqft) / 50)
          : 0.5,

      areaNormalized:
        minAreaSqft && minAreaSqft > 0
          ? Math.min(1, (warehouse.total_area || 0) / minAreaSqft)
          : 0.5,
      ...
      verifiedStatus: warehouse.amenities?.some(
        (a) =>
          a.toLowerCase().includes("verified") ||
      ...
      priceCompetitiveness:
        warehouse.price_per_sqft < 80
          ? (80 - warehouse.price_per_sqft) / 80 + 0.5
          : 0.5 - (warehouse.price_per_sqft - 80) / 200,
    };
    ...
    const rawOutput = hidden2.reduce(
      (sum, val, i) => sum + val * outputWeights[i],
      0,
    );
...
export function advancedEnsembleRecommend(
  ...
  if (district && district !== "any") {
    const locationMatched = warehouses.filter(
      (w) =>
  ...
  const rfWarehouses = randomForestRecommend(
    filteredWarehouses,
    preferences,
    50,
  );
  ...
  const weights = {
    knn: hasLocation ? 0.2 : 0.15,
    randomForest: 0.2,
    gradientBoosting: hasArea || hasPrice ? 0.25 : 0.2,
    neuralNetwork: 0.2,
    contentBased: 0.15,
  };
  ...
  const ensembleResults = filteredWarehouses.map((warehouse) => {
    ...
    const calibratedScore =
      boostedScore >= 0.7
        ...
        : boostedScore >= 0.5
          ? 0.7 + (boostedScore - 0.5) * 0.75 // Maps 0.5-0.7 to 0.70-0.85
          : 0.5 + boostedScore * 0.4; // Maps 0-0.5 to 0.50-0.70

    const finalScore = Math.min(0.99, Math.max(0.5, calibratedScore));

    // Calculate confidence based on algorithm agreement
    const scores = [knnScore, rfScore, gbData.score, nnScore, cbData.score];
    const variance =
      scores.reduce((sum, s) => sum + Math.pow(s - rawScore, 2), 0) /
      scores.length;
    ...
    if (hasArea) {
      ...
      if (availableArea >= minAreaSqft!) {
        reasons.push(
          `✅ Meets area requirement: ${availableArea.toLocaleString()} sqft available`,
        );
      }
    }
    ...
    if (reasons.length === 0) {
      reasons.push(
        `📊 ML-powered recommendation based on ${warehouse.amenities?.length || 0} features`,
      );
    }
 ...
 */
export function mapToRecommendedWarehouse(item: {
  warehouse: SupabaseWarehouse;
  score: number;
  reasons: string[];
  recommendationType: "ml" | "knn" | "random-forest" | "hybrid";
  ...
  const city =
    (warehouse as any).city || (warehouse as any).district || "Mumbai";

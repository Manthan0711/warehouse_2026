...
async function callGroq(prompt: string): Promise<LLMResponse | null> {
  ...
  try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert warehouse recommendation AI. Always respond with valid JSON only.",
            },
            {
              role: "user",
              content: prompt,
    ...
    console.log(
      `✅ Groq returned ${parsed.recommendations?.length || 0} recommendations`,
    );
...
async function callOpenRouter(prompt: string): Promise<LLMResponse | null> {
  ...
  try {
    ...
    console.log(
      `✅ OpenRouter returned ${parsed.recommendations?.length || 0} recommendations`,
    );
...
async function callGemini(prompt: string): Promise<LLMResponse | null> {
  ...
  try {
    ...
    const parsed = JSON.parse(jsonMatch[0]);
    console.log(
      `✅ Gemini returned ${parsed.recommendations?.length || 0} recommendations`,
    );

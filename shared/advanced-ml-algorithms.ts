...
function extractFeatures(
  ...
  return {
    ...
    available: Math.floor(
      (warehouse.total_area || 10000) * (1 - (warehouse.occupancy || 50) / 100),
    ),
...
function createFeatureVector(
  ...
  const normalizedPref = preferences.district
    ? normalizeDistrictName(preferences.district)
    : "";

  const locationMatch = preferences.district
    ? normalizedDistrict === normalizedPref || normalizedCity === normalizedPref
      ? 1.0
      : normalizedDistrict.includes(normalizedPref) ||
          normalizedPref.includes(normalizedDistrict)
        ? 0.85
        : normalizedCity.includes(normalizedPref) ||
            normalizedPref.includes(normalizedCity)
          ? 0.7
          : 0.2
    : 0.5;
  ...
  const areaMatch =
    minArea === 0
      ? 0.8
      : features.available >= minArea
        ? 1.0
        : features.available >= minArea * 0.8
          ? 0.7
          : 0.4;
  ...
  const availabilityScore =
    availabilityRatio > 0.1 && availabilityRatio < 0.8
      ? 1.0
...
function contentBasedScore(
  ...
  if (preferences.district) {
    ...
    const normalizedWarehouseDistrict = normalizeDistrictName(
      warehouse.district,
    );
    ...
    if (
      normalizedWarehouseDistrict === normalizedPref ||
      normalizedWarehouseCity === normalizedPref
    ) {
      score += locationWeight * 1.0;
    } else if (
      normalizedWarehouseDistrict.includes(normalizedPref) ||
      normalizedPref.includes(normalizedWarehouseDistrict)
    ) {
      score += locationWeight * 0.85;
    } else if (
      normalizedWarehouseCity.includes(normalizedPref) ||
      normalizedPref.includes(normalizedWarehouseCity)
    ) {
...
function collaborativeScore(
  ...
  const demandScore =
    availabilityRatio > 0.15 && availabilityRatio < 0.6 ? 0.25 : 0.1;

  // Price competitiveness
  const avgPrice =
    allWarehouses.reduce((sum, w) => sum + w.price, 0) / allWarehouses.length;
  const priceCompetitiveness = warehouse.price <= avgPrice ? 0.2 : 0.1;

  return Math.min(
    1,
    popularityScore + verifiedBoost + demandScore + priceCompetitiveness,
  );
}
...
function neuralScore(
  ...
  const hidden2 = [
    MathUtils.sigmoid(
      hidden1[0] * 1.2 + hidden1[1] * 0.8 + hidden1[2] * 0.6 - 0.5,
    ),
    MathUtils.sigmoid(
      hidden1[0] * 0.7 + hidden1[1] * 1.1 + hidden1[2] * 1.0 - 0.4,
    ),
  ];
...
function bayesianScore(
  ...
  if (preferences.district) {
    ...
    const normalizedWarehouse = normalizeDistrictName(
      warehouse.district || warehouse.city,
    );
    const normalizedPref = normalizeDistrictName(preferences.district);
    const locationMatch =
      normalizedWarehouse === normalizedPref ||
  ...
  const posterior =
    (prior * likelihood) /
    (prior * likelihood + (1 - prior) * (1 - likelihood) + 0.01);
...
export function advancedMLRecommend(
  ...
  console.log(
    `🧠 Running Advanced ML Algorithms on ${warehouses.length} warehouses...`,
  );
  ...
  if (preferences.district && preferences.district !== "any") {
    ...
    if (filteredWarehouses.length < limit) {
      ...
      filteredWarehouses = [
        ...filteredWarehouses,
        ...otherWarehouses.slice(0, limit - filteredWarehouses.length),
      ];
    }
  }
  ...
  const warehouseFeatures = filteredWarehouses.map((w) =>
    extractFeatures(w, filteredWarehouses),
  );

  // Score each warehouse using all algorithms
  const results: RecommendationResult[] = warehouseFeatures.map(
    (features, idx) => {
      ...
      const ensembleScore =
        ...
        algorithmScores.collaborative *
          ML_CONFIG.ensemble.weights.collaborative +
      ...
      if (preferences.district && preferences.district !== "any") {
        const normalizedPref = normalizeDistrictName(preferences.district);
        const normalizedWarehouse = normalizeDistrictName(
          features.district || features.city,
        );
        ...
        if (normalizedWarehouse === normalizedPref) {
          ...
          finalScore = Math.min(0.95, finalScore + 0.1); // Partial match bonus
        } else {
          finalScore = finalScore * 0.7; // Less harsh penalty for non-match
        }
      } else {
      ...
      if (algorithmScores.contentBased > 0.7) {
        if (
          ...
          features.price <= preferences.targetPrice
        ) {
          reasons.push(`Price (₹${features.price}/sqft) is within your budget`);
        }
        if (
          preferences.minAreaSqft &&
          features.available >= preferences.minAreaSqft
        ) {
          reasons.push(
            `Has ${features.available.toLocaleString()} sqft available space`,
          );
        }
      }
  ...
  console.log(
    `✅ Generated ${sortedResults.length} recommendations using Advanced 5-Algorithm Ensemble`,
  );
  if (sortedResults.length > 0) {
    console.log(
      `📊 Top result: ${sortedResults[0].warehouse.name} (Score: ${(sortedResults[0].score * 100).toFixed(1)}%)`,
    );
  }

...
export function PricingRecommendationModal({
  ...
  const fetchRecommendation = React.useCallback(async () => {
    ...
    try {
      ...
      const samples = (nearby || []).filter(
        (w) => w.price_per_sqft && w.price_per_sqft > 0,
      );
      const analyzedCount = samples.length;
      const avgPrice = analyzedCount
        ? Math.round(
            samples.reduce((sum, w) => sum + (w.price_per_sqft || 0), 0) /
              analyzedCount,
          )
        : 45;

      const avgOccupancyRaw = analyzedCount
        ? samples.reduce((sum, w) => sum + (w.occupancy || 0), 0) /
          analyzedCount
        : 0.6;
      const avgOccupancy =
        avgOccupancyRaw > 1 ? avgOccupancyRaw / 100 : avgOccupancyRaw;

      const demandScore = Math.min(
        10,
        Math.max(1, Math.round(avgOccupancy * 10)),
      );
      const amenityBoost =
        (warehouseData.amenities?.length || 0) >= 6 ? 1.08 : 1.03;
      const typeBoost = /cold|pharma|temperature/i.test(
      ...
      const sizeBoost =
        size >= 50000 ? 0.95 : size >= 20000 ? 0.98 : size <= 5000 ? 1.05 : 1.0;

      const baseRate = Math.round(
        avgPrice * typeBoost * amenityBoost * sizeBoost,
      );
...
{ "insight": "..." }`;
        ...
        const response = await getAIResponse({
          prompt,
          systemPrompt:
            "You create concise business-friendly pricing insights for warehouse owners. Return JSON only.",
          temperature: 0.3,
          maxTokens: 200,
        });
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Get AI-suggested pricing based on market analysis and competitor
            data
          </DialogDescription>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <Card className="border-2 border-primary">
              <CardContent className="pt-6">
                ...
                <div className="mt-4 flex items-center justify-center gap-6 text-sm">
                  ...
                  <div>
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${getDemandColor(result.demand_score)}`}
                    >
                ...
                <CardContent className="pt-6">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {result.market_data.nearby_warehouses_analyzed} warehouses
                      analyzed
                    </p>
                  </div>
                </CardContent>
            ...
            <p className="text-xs text-gray-500 text-center">
              ...
              This is an AI-generated suggestion. You can customize the final
              price.
            </p>
          </div>
        ) : (

...
export function GeminiChatbot({ className = "" }: ChatbotProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      type: "bot",
      content:
        "Hi! I'm your AI assistant for SmartSpace Warehouse powered by Claude 3.5 Sonnet. I can help you find the perfect warehouse, answer questions about facilities, pricing, and more. How can I help you today?",
      timestamp: new Date(),
  ...
  const generateGeminiResponse = async (
    ...
    if (
      ...
      message.includes("warehouses in")
    ) {
      ...
      const mentionedCity = maharashtraCities.find((city) =>
      ...
      if (mentionedCity) {
        ...
        const basePrice =
          mentionedCity === "mumbai" ? 80 : mentionedCity === "pune" ? 70 : 60;
        const warehouseCount =
          mentionedCity === "mumbai"
            ? 25000
            : mentionedCity === "pune"
              ? 18000
              : 12000;
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (isBookingRequest) {
        ...
        console.log(
          "🧠 Detected booking request, using Smart Booking Service...",
        );
        const bookingResult =
          await smartBookingService.processNaturalLanguageBooking(inputMessage);
    ...
    setMessages([
      {
        ...
        type: "bot",
        content:
          "Chat cleared! How can I help you with warehouse solutions today?",
        timestamp: new Date(),
      },
    ]);
  };
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 ${className}`}>
      <Card
        className={`w-96 transition-all duration-300 shadow-2xl border-0 ${

...
export default function ProtectedRoute({
  ...
  if (!profile) {
    console.log(
      "⚠️ ProtectedRoute: User authenticated but no profile found, redirecting to login",
    );
    return (
      <Navigate
        to="/login"
  ...
  if (allowedRoles && profile) {
    if (!allowedRoles.includes(profile.user_type)) {
      console.log(
        `🔒 ProtectedRoute: User role '${profile.user_type}' not in allowed roles [${allowedRoles.join(", ")}]`,
      );

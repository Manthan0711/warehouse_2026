...
interface WarehouseContextType {
  ...
  createWarehouse: (
    warehouse: Omit<Tables<"warehouses">, "id" | "created_at" | "updated_at">,
  ) => Promise<{ data: Warehouse | null; error: Error | null }>;
  updateWarehouse: (
    id: string,
    updates: Partial<Tables<"warehouses">>,
...
export const WarehouseProvider = ({ children }: WarehouseProviderProps) => {
    ...
    try {
      ...
      const enhancedWarehouses = data.map((wh: any) => ({
        ...
        available_area: Math.floor(
          (wh.total_area || 10000) * (1 - (wh.occupancy || 0.5)),
        ),
        occupancy_rate: Math.floor((wh.occupancy || 0.5) * 100),
        monthly_revenue: Math.floor(
          (wh.total_area || 10000) *
            (wh.price_per_sqft || 50) *
            (wh.occupancy || 0.7),
        ),
      }));
  ...
  const fetchWarehouseById = async (id: string): Promise<Warehouse | null> => {
    try {
      ...
      return {
        ...
        available_area: Math.floor(
          (data.total_area || 10000) * (1 - (data.occupancy || 0.5)),
        ),
        occupancy_rate: Math.floor((data.occupancy || 0.5) * 100),
        monthly_revenue: Math.floor(
          (data.total_area || 10000) *
            (data.price_per_sqft || 50) *
            (data.occupancy || 0.7),
        ),
      };
    } catch (err) {
  ...
  const fetchOwnerWarehouses = async (
    ownerId: string,
  ): Promise<Warehouse[]> => {
    try {
      const { data, error } = await supabase
      ...
      return data.map((warehouse) => ({
        ...
        monthly_revenue: Math.floor(
          warehouse.total_size_sqft * warehouse.pricing_inr_sqft_month * 0.7,
        ),
      }));
    } catch (err) {
    ...
    try {
      ...
      if (query.location) {
        supabaseQuery = supabaseQuery.or(
          `warehouse_address.ilike.%${query.location}%,city.ilike.%${query.location}%,district.ilike.%${query.location}%`,
        );
      }

...
export default function SeekerHub() {
    ...
    try {
      ...
      if (data.success && data.bookings) {
        ...
        const pending = data.bookings.filter(
          (b: Booking) => b.admin_status === "pending",
        ).length;
        const approved = data.bookings.filter(
          (b: Booking) => b.admin_status === "approved",
        ).length;
        const rejected = data.bookings.filter(
          (b: Booking) => b.admin_status === "rejected",
        ).length;
        const completed = data.bookings.filter(
          (b: Booking) => b.status === "completed",
        ).length;
        const cancelled = data.bookings.filter(
          (b: Booking) => b.status === "cancelled",
        ).length;
        const spent = data.bookings
          .filter(
            (b: Booking) =>
              b.admin_status === "approved" && b.payment_status !== "refunded",
          )
    ...
    try {
      ...
      if (data.success && data.warehouses && data.warehouses.length > 0) {
        ...
        if (localSaved.length > 0) {
          ...
          const warehousePromises = localSaved.map(
  ...
  const handleRemoveSaved = async (warehouseId: string) => {
    try {
      ...
      if (data.success && !data.saved) {
        ...
        setStats((prev) => ({
          ...prev,
          savedWarehouses: prev.savedWarehouses - 1,
        }));
        toast({
          title: "Removed",
  ...
  const filteredBookings = bookings.filter((booking) => {
    ...
    if (
      searchTerm &&
      !booking.warehouse_name.toLowerCase().includes(searchTerm.toLowerCase())
    )
      return false;
    return true;
  });
          ...
          .map((b) => (
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  {b.admin_status === "approved" ? (
                    <>
                      Your request for{" "}
                      <span className="font-semibold">{b.warehouse_name}</span>{" "}
              ...
              <button
                ...
                onClick={() =>
                  setDismissedNotifications((prev) => new Set([...prev, b.id]))
                }
              >
                <X className="h-4 w-4" />
              </button>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                ...
                <p className="text-gray-400">
                  Manage your bookings, saved warehouses, and activity from one
                  place
                </p>
              </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-4 mb-6 bg-slate-900/60 border border-slate-700/60">
            ...
            <TabsTrigger value="saved" className="flex items-center gap-2">
              ...
              {stats.savedWarehouses > 0 && (
                <Badge className="ml-1 bg-purple-500 text-xs">
                  {stats.savedWarehouses}
                </Badge>
              )}
            </TabsTrigger>
                        ...
                        >
                          ...
                          <div className="ml-3 text-right">
                            <p className="font-medium text-green-400">
                              ₹{saved.warehouse?.price_per_sqft}/sq ft
                            </p>
                          </div>
                        </div>
                      ))}
          ...
          <TabsContent value="bookings" className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              <Card
                className={`cursor-pointer transition-all ${bookingFilter === "all" ? "ring-2 ring-blue-500" : ""}`}
              ...
              <Card
                className={`cursor-pointer transition-all ${bookingFilter === "pending" ? "ring-2 ring-yellow-500" : ""}`}
              ...
              <Card
                className={`cursor-pointer transition-all ${bookingFilter === "approved" ? "ring-2 ring-green-500" : ""}`}
              ...
              <Card
                className={`cursor-pointer transition-all ${bookingFilter === "rejected" ? "ring-2 ring-red-500" : ""}`}
              ...
              <Card
                className={`cursor-pointer transition-all ${bookingFilter === "completed" ? "ring-2 ring-blue-500" : ""}`}
              ...
              <Card
                className={`cursor-pointer transition-all ${bookingFilter === "cancelled" ? "ring-2 ring-red-500" : ""}`}
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={() =>
                              navigate(
                                `/warehouses/${warehouse.wh_id || warehouse.id}`,
                              )
                            }
                          >
                            <Eye className="h-4 w-4 mr-1" />
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="font-medium">
                            ...
                            {booking.blocks_booked > 0 &&
                              ` - ${booking.blocks_booked} blocks`}
                          </p>
                          <p className="text-sm text-gray-400">

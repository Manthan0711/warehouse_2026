...
export default function Compare() {
  ...
  const availableWarehouses: ComparisonWarehouse[] = [
    ...
    },
    {
      ...
      type: "Logistics",
      image:
        "https://images.unsplash.com/photo-1601980169411-4c0d37967c2e?w=400&q=80",
      amenities: [
        "Rail Access",
        "Cold Storage",
        "Office Space",
    ...
    },
    {
      ...
      type: "Storage",
      image:
        "https://images.unsplash.com/photo-1553864250-05b20249ee6c?w=400&q=80",
      amenities: [
        "High Ceiling",
        "Truck Access",
        "Flexible Terms",
  ...
  const addWarehouse = (warehouse: ComparisonWarehouse) => {
    if (
      ...
      !selectedWarehouses.find((w) => w.id === warehouse.id)
    ) {
  ...
  const Car = ({ className }: { className?: string }) => (
    ...
    >
      <path
        ...
        strokeWidth={2}
        d="M8 7V3a1 1 0 011-1h6a1 1 0 011 1v4M3 12h18m-9 9a3 3 0 100-6 3 3 0 000 6zm8-3a3 3 0 100-6 3 3 0 000 6z"
      />
    </svg>
  );
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center space-x-2 text-sm text-gray-600 mb-6">
          ...
          <span>/</span>
          <Link to="/warehouses" className="hover:text-blue-600">
            Warehouses
          </Link>
          <span>/</span>
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600">
              Compare up to {maxComparisons} warehouses side by side to make the
              best decision
            </p>
          </div>
        ...
        {selectedWarehouses.length < maxComparisons && (
          <Card className="mb-8">
            <CardHeader>
              ...
              <CardDescription>
                You can compare up to {maxComparisons} warehouses. Currently
                selected: {selectedWarehouses.length}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {availableWarehouses
                  .filter(
                    (w) => !selectedWarehouses.find((sw) => sw.id === w.id),
                  )
                  .map((warehouse) => (
                    <div
                      key={warehouse.id}
        ...
        {selectedWarehouses.length > 0 ? (
          <div className="space-y-6">
            ...
            <div
              ...
              style={{
                gridTemplateColumns: `repeat(${selectedWarehouses.length}, 1fr)`,
              }}
            >
                ...
                <div className="space-y-8">
                  ...
                  <div>
                    ...
                    <div
                      className="grid gap-4"
                      style={{
                        gridTemplateColumns: `200px repeat(${selectedWarehouses.length}, 1fr)`,
                      }}
                    >
                      ...
                      {selectedWarehouses.map((w) => (
                        <div key={`${w.id}-total`} className="text-center">
                          {w.totalArea.toLocaleString()} sq ft
                        </div>
                      ))}
                      ...
                      {selectedWarehouses.map((w) => (
                        <div key={`${w.id}-deposit`} className="text-center">
                          ₹{w.pricing.securityDeposit.toLocaleString()}
                        </div>
                      ))}
                      ...
                      {selectedWarehouses.map((w) => (
                        <div key={`${w.id}-lease`} className="text-center">
                          {w.pricing.minimumLease}
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {selectedWarehouses.map((w) => (
                        <div key={`${w.id}-ceiling`} className="text-center">
                          {w.specifications.ceilingHeight}
                        </div>
                      ))}
                      ...
                      {selectedWarehouses.map((w) => (
                        <div key={`${w.id}-docks`} className="text-center">
                          {w.specifications.loadingDocks}
                        </div>
                      ))}
                  ...
                  <div>
                    ...
                    >
                      {Object.keys(selectedWarehouses[0].features).map(
                        (feature) => (
                          <>
                            <div
                              key={feature}
                            ...
                            >
                              {getFeatureIcon(
                                feature as keyof ComparisonWarehouse["features"],
                            ...
                            {selectedWarehouses.map((w) => (
                              ...
                              >
                                {w.features[
                                  feature as keyof typeof w.features
                                ] ? (
          ...
          <Card className="text-center py-16">
            <CardContent>
              ...
              <p className="text-gray-600 mb-6">
                Add warehouses from the selection above to start comparing
              </p>
              <Button asChild>

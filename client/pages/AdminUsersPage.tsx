...
export default function AdminUsersPage() {
  ...
  const [filterType, setFilterType] = useState<"all" | "seeker" | "owner">(
    "all",
  );
  ...
  const fetchActivity = async (user: UserProfile) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/user-activity?user_id=${user.user_id}&user_type=${user.user_type}`,
      );
  ...
  const deactivateUser = async (user: UserProfile) => {
    try {
      const table =
        user.user_type === "seeker" ? "seeker_profiles" : "owner_profiles";
      ...
      setUsers((prev) =>
        prev.map((u) =>
          u.id === user.id ? { ...u, verification_status: "disabled" } : u,
        ),
      );
    } catch (error) {
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-400">
                  {selectedUser.user_type} • {selectedUser.email}
                </p>
              </div>
              <Button
                variant="outline"

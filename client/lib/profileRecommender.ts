...
function calculateDistance(
  ...
  const priceTarget =
    (profile.budget_min || 20) + (profile.budget_max || 100) / 2;
  const areaTarget =
    (profile.required_area_min || 1000) +
    (profile.required_area_max || 100000) / 2;
  ...
  const locationMatch = profile.preferred_districts.includes(warehouse.district)
    ? 0
    : 1;

  // Type match (binary)
  const typeMatch = profile.preferred_warehouse_types.includes(
  ...
  const amenityMatch =
    requiredAmenities.length > 0
      ? 1 -
        requiredAmenities.filter((a) => warehouseAmenities.includes(a)).length /
...
function randomForestScore(
  ...
  if (
    warehouse.price_per_sqft >= minBudget &&
    warehouse.price_per_sqft <= maxBudget
  ) {
    score += 0.2;
  } else if (
    warehouse.price_per_sqft < minBudget * 1.2 ||
    warehouse.price_per_sqft < maxBudget * 1.2
  ) {
...
function collaborativeScore(
  ...
  const compatibleTypes = businessToWarehouseMap[profile.business_type] || [
    "General Storage",
  ];
...
export function ensembleRecommend(
  ...
  for (const warehouse of warehouses) {
    ...
    const ensembleScore = knn * 0.35 + rf * 0.4 + collab * 0.25;
    ...
    if (profile.preferred_districts.includes(warehouse.district)) {
      matchReasons.push(
        `Located in your preferred district: ${warehouse.district}`,
      );
    }
    ...
    if (warehouse.price_per_sqft <= maxBudget) {
      matchReasons.push(
        `₹${warehouse.price_per_sqft}/sqft is within your budget`,
      );
    }
...
`;
  ...
  try {
    ...
    const response = await getAIResponse({
      prompt,
      systemPrompt:
        "You are a warehouse recommendation expert. Provide concise, helpful explanations.",
      temperature: 0.7,
      maxTokens: 200,
    });
    return (
      response.text ||
      "ML analysis complete. Personalized recommendations based on your profile."
    );
  } catch (err) {

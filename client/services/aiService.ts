  ...
  for (const model of models) {
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(
          `  ❌ Groq ${model}: ${response.status} ${response.statusText} — ${errorBody.substring(0, 200)}`,
        );
      ...
      console.warn(
        `  ❌ Groq ${model}: ${err.name === "AbortError" ? "timeout" : err.message}`,
      );
      continue;
    }
  }
  ...
  for (const model of models) {
    try {
      ...
      if (!response.ok) {
        const errorBody = await response.text().catch(() => "");
        console.warn(
          `  ❌ OpenRouter ${model}: ${response.status} ${response.statusText} — ${errorBody.substring(0, 200)}`,
        );
        continue;
      }
      ...
      console.warn(
        `  ❌ OpenRouter ${model}: ${err.name === "AbortError" ? "timeout" : err.message}`,
      );
      continue;
    }
  }
...
Provide recommendations in JSON format:
{
  ...
  const response = await getAIResponse({
    prompt,
    systemPrompt:
      "You are a warehouse recommendation expert. Always respond with valid JSON.",
    temperature: 0.3,
    maxTokens: 2000,
  });
...
function generateLocalFallback(request: AIRequest): AIResponse {
  ...
  if (
    prompt.includes("listing description") ||
    prompt.includes("warehouse listing")
  ) {
  ...
  // Owner insights
    ...
    prompt.includes("owner insight") ||
    prompt.includes("owner dashboard")
  ) {
    text =
      "Your warehouse portfolio is performing well in the current market. Consider optimizing pricing for peak season demand in Maharashtra. Tip 1: List detailed amenities and high-quality images to improve inquiry rates by up to 40%. Tip 2: Offer flexible rental terms (monthly/quarterly) to attract a wider range of tenants.";
  }
  // Chatbot / general
  else if (prompt.includes("warehouse") || prompt.includes("storage")) {
    text =
      "I can help you find the right warehouse. Our platform lists over 10,000 warehouses across Maharashtra with prices ranging from ₹25-150 per sq ft. Tell me your preferred city, required area, and storage type, and I'll suggest the best options for you.";
  }
  // Generic fallback
  else {
    text =
      "I'm SmartSpace AI Assistant. I can help with warehouse recommendations, pricing insights, and platform guidance. Please ask me about available warehouses, pricing, or how to list your property.";
  }

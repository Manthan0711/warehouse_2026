...
function inferTypeFromGoodsHeuristic(goodsType: string): string | undefined {
  ...
  if (
    /(vaccine|vaccines|insulin|cold\s*chain|frozen|ice\s*cream|seafood|meat|dairy|fruit|fruits|vegetable|vegetables|perishable|perishables)/i.test(
      g,
    )
  ) {
    return "Cold Storage";
  ...
  if (
    /(medicine|medicines|pharma|pharmaceutical|medical|clinic|hospital|lab|biotech)/i.test(
      g,
    )
  ) {
  ...
  if (
    /(chemical|chemicals|hazard|paint|solvent|industrial\s*fluids)/i.test(g)
  ) {
...
async function inferPreferredTypeFromGoodsType(
  goodsType: string,
...
{ "preferredType": "one of the listed warehouse types" }`;
    ...
    const response = await getAIResponse({
      prompt,
      systemPrompt:
        "You map goods types to the most suitable warehouse type. Always respond with valid JSON only.",
      temperature: 0.1,
      maxTokens: 150,
    });
    ...
    if (jsonMatch) {
      ...
      if (
        ...
        WAREHOUSE_TYPES.includes(parsed.preferredType)
      ) {
...
async function getSemanticScores(
  ...
  warehouses.forEach((w) => {
    ...
    if (
      ...
      location.includes(city.replace(" city", ""))
    ) {
      score += 30;
    ...
    if (requirement.preferredType && w.warehouse_type) {
      if (
        w.warehouse_type
          .toLowerCase()
          .includes(requirement.preferredType.toLowerCase())
      ) {
        score += 15;
      }
    }
...
async function getAvailableBlocks(
  ...
  console.log(
    "🔍 Smart Booking: Searching warehouses in",
    location,
    "(terms:",
    searchTerms.join(", "),
    ")",
  ...
  try {
    ...
    console.log(
      `📊 Working with ${allWarehouses.length} warehouses from cache/database`,
    );
    ...
    const availableWarehouses = allWarehouses.filter(
      (w) =>
        (w.occupancy === null ||
          w.occupancy === undefined ||
          w.occupancy < 1) &&
        w.total_area > 0,
    );

    console.log(
      `📦 ${availableWarehouses.length} warehouses have available space`,
    );

    // Filter by location in JavaScript for flexible matching
    const matchedWarehouses = availableWarehouses
      .filter((w) => {
    ...
    console.log(
      `✅ Found ${matchedWarehouses.length} warehouses matching "${location}"`,
    );
    ...
    for (const warehouse of matchedWarehouses) {
      ...
      const blockSize = Math.min(
        400,
        Math.max(100, Math.round(availableArea / 20)),
      ); // Smaller blocks
      const numBlocks = Math.max(
        1,
        Math.min(15, Math.floor(availableArea / blockSize)),
      ); // More blocks

      for (let i = 0; i < numBlocks; i++) {
        const blockNumber = i + 1;
        // Vary block sizes slightly for realism (90-110% of base size)
        const actualBlockSize = Math.round(
          blockSize * (0.9 + Math.random() * 0.2),
        );
    ...
    console.log(
      `📦 Generated ${allBlocks.length} blocks from ${matchedWarehouses.length} warehouses`,
    );
...
Respond in JSON format:
{
  ...
  try {
    const response = await getAIResponse({
      prompt,
      systemPrompt:
        "You are a warehouse booking optimization expert. Always respond with valid JSON.",
      temperature: 0.3,
      maxTokens: 800,
    });
...
function findSingleWarehouseOptions(
  ...
  warehouseBlocks.forEach((wBlocks, warehouseId) => {
    ...
    if (totalArea >= requirement.requiredSpace * 0.9) {
      // Allow 10% flexibility
      const totalCost = selectedBlocks.reduce(
        (sum, b) => sum + b.areaSqft * b.pricePerSqft,
        0,
      );
      ...
      if (!requirement.maxBudget || avgPrice <= requirement.maxBudget)
        matchScore += 10;
      ...
      options.push({
        ...
        cons:
          totalArea < requirement.requiredSpace
            ? [
...
function findMergedWarehouseOptions(
  ...
  if (
    totalArea >= requirement.requiredSpace * 0.9 &&
    ...
    if (warehouseGroups.size > 1) {
      ...
      const singleWarehouseCost = selectedBlocks.reduce(
        (sum, b) =>
          sum +
          b.areaSqft *
            Math.max(
              ...blocks
                .filter(
                  (bl) => bl.warehouseId === selectedBlocks[0].warehouseId,
                )
                .map((bl) => bl.pricePerSqft),
            ),
        0,
      );
      const savings =
        singleWarehouseCost > totalCost ? singleWarehouseCost - totalCost : 0;

      const warehouses = Array.from(warehouseGroups.entries()).map(
        ([whId, whBlocks]) => ({
          id: whId,
      ...
      options.push({
        ...
        matchScore: Math.min(
          ...
          75 +
            (savings > 0 ? 15 : 5) +
            (totalArea >= requirement.requiredSpace ? 5 : 0),
        ),
...
export async function analyzeBookingRequirements(
  ...
  if (requirement.goodsType && !requirement.preferredType) {
    const inferredType = await inferPreferredTypeFromGoodsType(
      requirement.goodsType,
    );
    if (inferredType) {
  ...
  console.log(
    `📦 Found ${availableBlocks.length} available blocks in ${requirement.location}`,
  );
  ...
  if (requirement.preferredType) {
    ...
    console.log(
      `🏷️ Filtered blocks by type (${requirement.preferredType}): ${availableBlocks.length}`,
    );
  }
  ...
  if (availableBlocks.length < 10 && requirement.flexibleLocation) {
    ...
    if (available.length > 0) {
      for (const warehouse of available) {
        ...
        const blockSize = Math.min(
          1000,
          Math.max(200, Math.round(availableArea / 10)),
        );
        const numBlocks = Math.max(
          1,
          Math.min(5, Math.floor(availableArea / blockSize)),
        );
  ...
  const singleOptions = findSingleWarehouseOptions(
    availableBlocks,
    requirement,
  );
  const mergedOptions = findMergedWarehouseOptions(
    availableBlocks,
    requirement,
  );
  ...
  if (
    ...
    const relaxedSingleOptions = findSingleWarehouseOptions(
      availableBlocks,
      relaxedRequirement,
    );
    const relaxedMergedOptions = findMergedWarehouseOptions(
      availableBlocks,
      relaxedRequirement,
    );
    ...
    allOptions = [...relaxedSingleOptions, ...relaxedMergedOptions]
      ...
      .map((opt) => ({
        ...
        cons: [
          ...opt.cons,
          `Price (₹${opt.averagePricePerSqft}/sq ft) exceeds budget (₹${requirement.maxBudget}/sq ft)`,
        ],
      }));

    console.log(
      `📊 Found ${allOptions.length} alternative options (above budget)`,
    );
  }
  ...
  // Get LLM analysis
  const llmAnalysis = await getLLMBookingAnalysis(
    requirement,
    availableBlocks,
    allOptions,
  );
  ...
  if (llmAnalysis.bestOptionId) {
    const bestIndex = allOptions.findIndex(
      (opt) => opt.id === llmAnalysis.bestOptionId,
    );
    if (bestIndex > 0) {
...
export async function processNaturalLanguageBooking(
  ...
  requirement: BookingRequirement | null;
  analysis: BookingAnalysis | null;
  response: string;
}> {
...
- "Need space for vaccines in Hyderabad" → location: "Hyderabad", goodsType: "vaccines"`;
  ...
  try {
    const extractResponse = await getAIResponse({
      ...
      systemPrompt:
        "You are a booking request parser. Extract structured data from natural language. Always respond with valid JSON.",
      temperature: 0.1,
      maxTokens: 500,
    });
    ...
    if (!jsonMatch) {
      return {
        ...
        analysis: null,
        response:
          "I couldn't understand your booking request. Could you please specify:\n- How much space you need (in sq ft)\n- Which city/location\n- Your budget (if any)",
      };
    }
    ...
    if (!parsed.isBookingRequest) {
      return {
        ...
        analysis: null,
        response:
          "This doesn't seem to be a booking request. I can help you book warehouse space! Just tell me:\n- Required space (sq ft)\n- Location/city\n- Budget preference",
      };
    }
    ...
    if (requirement.maxBudget)
      response += `- Budget: ₹${requirement.maxBudget}/sq ft/month\n`;
    if (requirement.preferredType)
      response += `- Type: ${requirement.preferredType}\n`;
    if (requirement.goodsType)
      response += `- Goods: ${requirement.goodsType}\n`;
    response += `\n`;
    ...
    if (analysis.options.length > 0) {
      ...
      if (best.savings)
        response += `- 💰 You Save: ₹${best.savings.toLocaleString()}/month!\n`;
      response += `\n`;
    ...
    return {
      ...
      analysis: null,
      response:
        "I encountered an error processing your request. Please try again with details like: space needed, location, and budget.",
    };
  }
}
...
Return JSON:
{
  ...
  try {
    const response = await getAIResponse({
      prompt,
      systemPrompt:
        "You are a warehouse space optimization expert. Select optimal blocks. Respond with valid JSON only.",
      temperature: 0.1,
      maxTokens: 300,
    });
    ...
    if (jsonMatch) {
      ...
      const selected = availableOnly.filter((b) =>
        result.selectedBlocks.includes(b.blockNumber),
      );
  ...
  const sorted = [...availableOnly].sort(
    (a, b) => a.pricePerSqft - b.pricePerSqft || a.blockNumber - b.blockNumber,
  );
